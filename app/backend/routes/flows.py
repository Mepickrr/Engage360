...
def _summary(doc: dict) -> dict:
    ...
    return {
        ...
        # Fields surfaced on the redesigned /flows list page.
        ...
        "revenue_inr": doc.get("revenue_inr"),
        "spent_inr": doc.get("spent_inr"),
    }
...
async def list_flows(request: Request):
    ...
    docs = (
        await db.flows.find({"deleted": {"$ne": True}}, {"_id": 0})
        .sort("updated_at", -1)
        .to_list(length=200)
    )
...
async def get_flow(flow_id: str, request: Request):
    db = request.app.state.db
    doc = await db.flows.find_one({"id": flow_id}, {"_id": 0})
    if not doc:
