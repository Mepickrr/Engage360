    ...
    <div className="space-y-3" data-testid={`config-node-${type}`}>
      ...
      {type === "wait" && (
        <NumberInput
          ...
          onChange={(v) => patch({ duration_minutes: v })}
          testId="config-wait-duration"
          suffix="min"
        />
      )}
      ...
      {type === "generic" && (
        <div
          className="text-[12px] text-text-muted italic border border-dashed border-border rounded-md px-3 py-4 text-center"
          data-testid="config-generic-placeholder"
        >
