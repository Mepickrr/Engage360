...
function OrchestratorTeamViewV4({ agents, onAgentClick, onBuildClick }) {
  injectCss("v5-flow-css", FLOW_CSS);
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            animation: "v5-orch-pulse 3s ease-in-out infinite",
        ...
        >
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#4ade80", display: "inline-block", boxShadow: "0 0 6px #4ade80", animation: "v5-orch-pulse 2s ease-in-out infinite" }} />
        ...
        <div style={{ height: 64, width: "100%", position: "relative", zIndex: 1 }}>
          {paths.length > 0 && (
            ...
            >
              <defs>
                <filter id="v5-glow" x="-50%" y="-50%" width="200%" height="200%">
                ...
                return (
                  <g key={i}>
                    <path d={d} fill="none" stroke={color} strokeWidth={1}
                      strokeDasharray="4 6"
                      style={{ animation: `v5-shimmer ${1.8 + i * 0.25}s ease-in-out infinite`, animationDelay: `${DELAYS[i] ?? 0}s` }}
                    />
                    <path d={d} fill="none" stroke={color} strokeWidth={2.5}
                      strokeLinecap="round" filter="url(#v5-glow)"
                      style={{ "--pl": "280", strokeDasharray: "7 280", strokeDashoffset: 280,
                        animation: `v5-packet ${1.5 + i * 0.18}s cubic-bezier(0.4,0,0.6,1) infinite`,
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", zIndex: 2, position: "relative" }}>
          {orderedAgents.map(({ pillar, agent }, i) => {
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  data-testid={`v5-team-card-${agent.id}`}
                  ...
                  style={{
                    ...
                    animation: `v5-agent-ping 0.5s ease-out both`,
          ...
          <div style={{ width: 152, display: "flex", flexDirection: "column", alignItems: "center", gap: 0 }}>
            ...
            <button
              type="button"
              data-testid="v5-build-agent-card"
...
export default function HomeV5Page() {
  const { data: me } = useQuery({
    ...
    staleTime: 5 * 60_000,
    retry: 1,
    onError: (e) => { console.warn("HomeV5 fetchMe failed:", e?.message); },
  });
  const firstName = me?.user?.name?.split(" ")[0] || "there";

  return (
    <div className="space-y-6 animate-fade-in-up max-w-[1400px] mx-auto" data-testid="page-home-v5">
      ...
      {/* Ask AI — expanded prominent section */}
      <section>
        <div className="mb-3">
