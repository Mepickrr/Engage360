...
export default function FlowBuilderV2() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: (doc) => {
      ...
      navigate(`/flows-v2/builder/${doc.id}`, { replace: true });
    },
    onError: () => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (saved?.id && saved.id !== flowId) {
          setFlowId(saved.id);
          navigate(`/flows-v2/builder/${saved.id}`, { replace: true });
        }
  ...
  return (
    <FlowVariantContext.Provider value={v2Variant}>
    <>
      <div
        className="flex flex-col h-[calc(100vh-3rem)] -m-6 bg-app-bg"
        data-testid="flow-builder-v2"
      >
        <BuilderTopbar basePath="/flows-v2" />
        <div className="flex-1 flex min-h-0">
          <NodePalette onNodeAdd={handlePaletteNodeAdd} allowedNodeIds={V2_ALLOWED_NODES} />
