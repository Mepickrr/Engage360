...
import { AGENT_META, getAgentMeta, hexAlpha } from "@/lib/agentMeta";
...
// ─── Static agent data — no backend required ──────────────────
// Built from AGENT_META; avatar_initials derived from agent names.
const STATIC_AGENTS = [
  { ...AGENT_META.meera, avatar_initials: "ME" },
  { ...AGENT_META.aryan, avatar_initials: "AR" },
  { ...AGENT_META.zara,  avatar_initials: "ZA" },
  { ...AGENT_META.dev,   avatar_initials: "DV" },
  { ...AGENT_META.rishi, avatar_initials: "RI" },
  { ...AGENT_META.priya, avatar_initials: "PR" },
];
...
function OrchestratorTeamView({ agents, onAgentClick, onBuildClick }) {
  injectCss("v6-flow-css", FLOW_CSS);
  ...
  return (
    <section data-testid="orchestrator-team-v6" style={{ position: "relative" }}>
      <div
        ref={containerRef}
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            animation: "v6-orch-pulse 3s ease-in-out infinite",
        ...
        >
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#4ade80", display: "inline-block", boxShadow: "0 0 6px #4ade80", animation: "v6-orch-pulse 2s ease-in-out infinite" }} />
            <span style={{ fontSize: 11, opacity: 0.85 }}>live</span>
          </div>
        </div>

        {/* ── SVG connector (orch → pillars) ── */}
                ...
                return (
                  <g key={i}>
                    <path d={d} fill="none" stroke={color} strokeWidth={1}
                      strokeDasharray="4 6"
                      style={{ animation: `v6-shimmer ${1.8 + i * 0.25}s ease-in-out infinite`, animationDelay: `${DELAYS[i] ?? 0}s` }}
                    />
                    <path d={d} fill="none" stroke={color} strokeWidth={2.5}
                      strokeLinecap="round" filter="url(#v6-glow)"
                      style={{ "--pl": "280", strokeDasharray: "7 280", strokeDashoffset: 280,
                        animation: `v6-packet ${1.5 + i * 0.18}s cubic-bezier(0.4,0,0.6,1) infinite`,
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", zIndex: 2, position: "relative" }}>
          {orderedAgents.map(({ pillar, agent }, i) => {
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  data-testid={`v6-team-card-${agent.id}`}
                  ...
                  style={{
                    ...
                    animation: `v6-agent-ping 0.5s ease-out both`,
          ...
          <div style={{ width: 152, display: "flex", flexDirection: "column", alignItems: "center", gap: 0 }}>
            ...
            <button
              type="button"
              data-testid="v6-build-agent-card"
...
function MeetTheTeam() {
  ...
  // All agent data is static — no backend dependency.
  const allAgents = [...STATIC_AGENTS, ...customAgents];
  ...
  return (
    <section data-testid="meet-the-team-v6">
...
export default function HomeV6Page() {
  return (
    <div className="space-y-6 animate-fade-in-up max-w-[1400px] mx-auto" data-testid="page-home-v6">
      ...
      <section>
        <div className="mb-3">
          <h2 className="text-base font-semibold text-text-primary">Chat with your AI team</h2>
          <p className="text-xs text-text-muted mt-0.5">Ask anything — your agents will collaborate to answer</p>
        </div>
        <div className="rounded-2xl border border-border bg-gradient-to-br from-white to-[#FAF8FF] shadow-[0_2px_16px_rgba(108,58,232,0.08)]">
          <AskAiBar large />
        </div>
      </section>
    </div>
