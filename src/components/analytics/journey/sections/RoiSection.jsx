...
export default function RoiSection({ data, isLoading }) {
  if (isLoading) return <SectionSkeleton testId="journey-roi-skeleton" rows={3} />;

  return (
    <div data-testid="journey-roi-section" className="space-y-3">
      ...
      <div className="bg-surface border border-border rounded-lg p-4" data-testid="journey-roi-overall">
      ...
      <div className="grid grid-cols-3 gap-2" data-testid="journey-roi-by-channel">
        {data.roi.byChannel.map((c) => (
          <div key={c.channel} className="bg-slate-50 rounded-md p-2 text-center" data-testid={`journey-roi-channel-${c.channel.toLowerCase()}`}>
