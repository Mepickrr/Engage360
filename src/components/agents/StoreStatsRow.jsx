...
export default function StoreStatsRow({ compact = false }) {
  ...
  return (
    <div
      className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 ${compact ? "gap-2.5" : "gap-4"}`}
      ...
      {KPI_DEFS.map((kpi) => {
        ...
        return (
          <div
            ...
            className={`bg-surface border border-border rounded-lg transition-shadow hover:shadow-sm ${compact ? "p-3" : "p-4"}`}
          >
            ...
            <div className={`${compact ? "mt-1 text-xl" : "mt-2 text-2xl"} font-semibold text-text-primary tabular-nums`}>
