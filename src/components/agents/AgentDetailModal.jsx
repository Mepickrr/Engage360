...
import { SendHorizontal, X, Settings } from "lucide-react";
import { getAgentMeta, hexAlpha } from "@/lib/agentMeta";
import { useConversationStore } from "@/store/uiStore";
import AgentSettingsPanel from "./AgentSettingsPanel";

export default function AgentDetailModal({ agentId, agents, onClose, onAgentSaved }) {
  const open = !!agentId;
  ...
  // Sync local agent copy when agentId changes
  useEffect(() => {
    const found = safeAgents.find((a) => a.id === agentId) || null;
    setLocalAgent(found);
    setText("");
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* CHANGE 2: gear icon left of ✕ */}
          <div className="absolute right-3 top-3 flex items-center gap-1">
            <button
              type="button"
              onClick={() => setSettingsOpen(true)}
              data-testid={`agent-modal-settings-${agent.id}`}
              className="p-1 rounded-md text-text-muted hover:text-primary hover:bg-slate-100 transition-colors"
          ...
          >
            <input
              ...
              placeholder={meta?.askPlaceholder || `Ask ${agent.name}…`}
      ...
      {/* CHANGE 2: Settings panel slides in */}
      <AgentSettingsPanel
        open={settingsOpen}
