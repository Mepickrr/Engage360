...
// Module-scope to prevent React unmounting on each parent re-render.
...
export default function ConditionalSplitNode({ id, data, selected }) {
  ...
  // Build port list based on active mode
  const ports = [];
  if (mode === "filter") {
    ...
    exprs.forEach((e, i) => {
      ...
      ports.push({ id: e.id, label: raw.length > 22 ? raw.slice(0, 20) + "…" : raw, color: TEAL });
    });
    ...
    >
      ...
      {isEmpty ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px 16px", gap: 8 }}>
          ...
          <span style={{ fontSize: 13, color: "#94A3B8", fontWeight: 500 }}>Conditional Split</span>
        ...
        <>
          <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 12px" }}>
            <div style={{ width: 24, height: 24, borderRadius: 6, background: TEAL, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <GitFork size={13} color="#fff" />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 9, fontWeight: 700, color: TEAL, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                Conditional Split
              </div>
            ...
            {modeLabel && (
              <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 7px", borderRadius: 10, background: "#CCFBF1", color: TEAL, flexShrink: 0 }}>
                {modeLabel}
              </span>
            )}
          </div>
